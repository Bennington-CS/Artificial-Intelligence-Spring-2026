...
(define (scale-inputs raw-features)
  (let ([range1 (first feature-ranges)]
    ...
    (list (min-max-scale (first raw-features)  (car  range1) (cadr range1))
          (min-max-scale (second raw-features) (car  range2) (cadr range2)))))
...
(define (perceptron-output scaled-inputs weights bias)
  (sigmoid (+ (dot-product scaled-inputs weights) bias)))
...
(define (perceptron-update example weights bias lr)
  (let* ([scaled   (scale-inputs (features example))]
         ...
         [error    (- actual output)]
         [lr-error (* lr error)])
    (values (list (+ (first weights)  (* lr-error (first scaled)))
                  (+ (second weights) (* lr-error (second scaled))))
            (+ bias lr-error))))
...
(define (perceptron-train data lr epochs)
  (let loop-epochs ([epoch   0]
                    [weights '(0.0 0.0)]
                    [bias    0.0])
    (if (equal? epoch epochs)
        (values weights bias)
        (let loop-examples ([remaining data]
                            [weights   weights]
                            [bias      bias])
          (if (empty? remaining)
              (loop-epochs (add1 epoch) weights bias)
              (let-values ([(new-weights new-bias)
                            (perceptron-update (first remaining) weights bias lr)])
                (loop-examples (rest remaining) new-weights new-bias)))))))
...
(define (perceptron-accuracy data weights bias)
  (length (filter (lambda (example)
                    (let ([scaled (scale-inputs (features example))])
                      (equal? (classify (perceptron-output scaled weights bias))
                              (label example))))
                  data)))
...
(define (perceptron-classify-all data weights bias)
  (map (lambda (example) (classify (perceptron-output (scale-inputs (features example))
                                                      weights bias)))
       data))
